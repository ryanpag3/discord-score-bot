...
const routeMessage = async (message: Message) => {
    ...
    try {
        ...
        if (!cmdInfo && isShorthandMessage(command)) {
            return await routeShorthandMessage(command, message);
        } else if (!cmdInfo) {
            throw new Error(`Invalid command provided.`);
        }
        await commands[cmdInfo.filename](command, message);
    } catch (e) {
        handleCommandError(command, e.message, message);
    }
}

const routeShorthandMessage = async (command: string, message: Message) => {
    if (command.endsWith(`++`)) {
        return await plus(command, message);
    } else if (command.endsWith(`--`)) {
        return await minus(command, message);
